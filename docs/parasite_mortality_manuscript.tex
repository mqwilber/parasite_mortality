...
\textbf{Keywords:} & aggregation, negative binomial distribution, parasite intensity, Crofton Method, Likelihood Method
\\
...
parasite-induced mortality in wildlife populations. Using simulations, we show
...
However, we stress that this method, and all techniques that estimate parasite-
induced mortality from intensity data alone, have several critical assumptions
that limit their applicability in the real world. Due to these limitations,
these methods should only be used as an exploratory tool to inform more
rigorous studies of parasite-induced host mortality.

\textbf{Keywords}: parasite aggregation, negative binomial distribution
...
Infectious agents can impact on animal populations through changing
...
probability; an important relationship for determining the potential impact of
a parasite on host dynamics \citep{AndersonandMay1978s}. \cite{Adjei1986}
...
description of the Adjei Method). \citeauthor{Adjei1986} noted that this
survival function could provide useful information such as the parasite
intensity at which a host has a greater than 50\% chance of suffering parasite-
induced mortality. However, if mean infection intensity is high or sample sizes
...
from intensity data and the place of any of these methods in modern quantitative parasitology.

\section*{Methods}

\subsection*{A novel, likelihood-based method for estimating PIHM}

Our method (henceforth the Likelihood Method) begins with the same assumptions as the Adjei Method: namely that infection of
...
\citeauthor{Adjei1986}, this is not necessarily an unrealistic as some parasite infections are host age dependent with mortality occurring soon after infection \citep{Schotthoefer2003,Johnson2008}. [CHAT WITH SARA ABOUT THIS]
...
more aggregation) \citep{Crofton1971a,AndersonandMay1978,Adjei1986}. A variety of different biological and statistical assumptions can result in an equilibrium parasite distribution that follows a negative binomial distribution
...
\citep{AndersonandMay1978,Dobson1992,Barbour2000}. In systems where there is truly a linear relationship between infection intensity and survival probability it will be nearly impossible to use intensity data to detect parasite-induced host mortality \citep{Lanciani1989}.  However,  there is substantial empirical evidence that host-parasite systems do not universally exhibit linear host-survival functions \citep{Benesh2011}, suggesting a linear form for $h(\text{survival} ; x, \boldsymbol{\theta}$) is not always the most reasonable.
...
Although superior to the Adjei Method, the Likelihood Method still cannot be applied to all real datasets. For host-parasite systems where host mortality occurs as a steep, non-linear function of parasite intensity only 80 hosts must be sampled to have an 80\% power in detecting PIHM. However, as the maximum slope of the survival function decreases and the function becomes, hundreds, or possibly thousands of hosts would have to be sampled to achieve the same result. This is consistent with previous studies which illustrate the difficulty of detecting PIHM from linear host survival functions \citep{Lanciani1989}.  While it may be feasible to sample several hundred invertebrates or small fish, even the smallest sample sizes are completely
...
We thank Hamish McCallum for discussions on the inadequacy of estimating PIHM from intensity data and Kevin Lafferty for manuscript comments. We also thank the Briggs Lab Group and Theoretical Ecology group at UCSB for helpful feedback.  This work was supported by X and X.
...
\begin{table}
    ...
    $a$ & When $b$ is held constant a one unit increase in $a$ leads to a $1 / b$ increase in the parasite intensity at which some percent of hosts survive\\
...
\begin{figure}
    ...
    \begin{tikzpicture}
        \node at (0, 0) (hist) {\includegraphics[width=\textwidth]{/Users/mqwilber/Repos/parasite_mortality/results/concept_crofton_method}};

        % %\node at (2, -2) (trunc) {Truncated tail due to PIHM};
        % %\draw[->] (trunc.south)--(-1, -3);

        % % \foreach \x in {2,...,4}
        % %     \draw[ultra thick, dashed, opacity=(1 / \x)] (-3.5 + \x - 1, -3.68)--(-3.5 + \x - 1, 4.75) node[midway, rotate=90, fill=white,opacity=1] {$\text{iter}_{\x}$: Pre $\neq$ Post};

        % \draw[ultra thick, dashed, opacity=0.2] (1.5, -3.68)--(1.5, 4.75) node[midway, rotate=90, fill=white,opacity=1] (ncm) {Pre $\neq$ Post};

        % \draw[ultra thick, dashed] (-2, -3.68)--(-2, 4.75) node[midway, rotate=90, fill=white, opacity=1] (cm) {Pre = Post};

        % \draw[->] (ncm.north)--(cm.south) node[midway, above=0.1cm, align=center, fill=white] {\tiny{Change truncation}\\\tiny{until pre = post}\\\tiny{to estimate pre-mortality}\\\tiny{distribution}};

        % \draw[->] (cm.north)--(-3, 0.55);



        % %\draw[<-, ultra thick, opacity=0.5] (-4, 3)--(-3.5, 3);
