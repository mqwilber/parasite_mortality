...
parasite-induced mortality in wildlife populations. We show
...
However, we stress that this method, and all techniques that estimate parasite-induced mortality from intensity data alone, have several critical assumptions
...
\textbf{Highlights}
\begin{itemize}
    \item We develop and test a novel method for detecting and quantifying parasite-induced host mortality from parasite intensity data.
    \item Our method outperforms previous methods for detecting parasite-induced host mortality.
    \item Our method correctly quantifies parasite-induced host mortality in empirical data where mortality has been qualitatively reported
    \item  All current methods for detecting parasite-induced host mortality have fundamental and untestable assumptions that limit their applicability to many datasets.
\end{itemize}
...
description of the Adjei Method).  This
survival function can provide useful information such as the parasite
intensity at which a host has a greater than 50\% chance of suffering parasite-induced mortality. However, if mean infection intensity is high or sample sizes
...
distribution with parameters mean parasite intensity ($\mu_p$) and
aggregation ($k_p$) before mortality \citep{Crofton1971a,AndersonandMay1978,Adjei1986}. A variety of different biological and statistical assumptions can result in an equilibrium parasite distribution that follows a negative binomial distribution
