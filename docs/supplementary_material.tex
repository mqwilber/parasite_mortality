...
\textbf{1 Implementation of the Crofton Method}
...
p_{max}$. Truncate the data such that $\text{data}_\text{truncated} \leq t_2$ and repeat the above
...
\textbf{2 Implementation of the Adjei Method}
...
To estimate the parameters in equation 1, the Adjei Method first
...
\citep{Adjei1986}.  After this questionable manipulation, the (observed, predicted) pairs are fit to a standard Generalized Linear Model \citep{McCullagh1989} with a binomial response variable and a logistic link function given by equation 1.  This model provides estimates for parameters $a$, $b$ and $LD_{50}$.
...
\textbf{3 Code and unit tests for estimating parasite-induced host mortality}
